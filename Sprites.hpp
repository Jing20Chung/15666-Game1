...
struct Sprites {
    ...
    static std::vector< Sprite > all_sprite; // all found sprite, sprite contains: tile number, palette table number, and name.
	static std::array< uint16_t, BackgroundWidth * BackgroundHeight > background;

    static Sprites load(std::string const &filename);
    static Sprite lookup(std::string const &sprite_name);
};